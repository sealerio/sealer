...
func (d *Default) joinNodes(nodes []string) error {
	...
	cmdAddRegistryHosts := fmt.Sprintf(RemoteAddEtcHosts, d.getRegistryHost())
...
func (d *Default) deleteNode(node string) error {
	...
	if err := d.SSH.CmdAsync(host, fmt.Sprintf(RemoteCleanMasterOrNode, vlogToStr(d.Vlog)), fmt.Sprintf(RemoteRemoveAPIServerEtcHost, d.APIServer), fmt.Sprintf(RemoteRemoveAPIServerEtcHost, d.getRegistryHost())); err != nil {
		return err
	}
