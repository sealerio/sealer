...
`CNI --type calico / flannel --cidr ...` a easiest way to deploy cni, sealer will support several types of cni.

`CNI --source [path / https://...]`. `path` is a relative path refers to local build context, this can be a directory
contains multiple yaml or a single yaml for deploying cni. `https://...` is a remote address for downloading related files to deploy cni.
...
`CSI --type alibaba-cloud-csi-driver ` a easiest way to deploy csi, sealer will support several types of csi.

`CSI --source [path / https://...]`. `path` is a relative path refers to local build context, this can be a directory
contains multiple yaml or a single yaml for deploying csi. `https://...` is a remote address for downloading related files to deploy csi.
...
`APP [UNIQUE-NAME] --source [path / https://...]`. `[UNIQUE-NAME]` is a declaration for an application, which is unique. `path` is a relative path refers to local build context, this can be a directory
contains multiple yaml or a single file for deploying app. `https://...` is a remote address for downloading related files to deploy app.

`[]` means user could declare several sources

### LAUNCH

`LAUNCH` allows user to specify which apps to start right after the completion of cluster initing.
