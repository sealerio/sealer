...
func SetUpClient() (*container2.DockerProvider, error) {
	cluster := &v1.Cluster{
	...
	client, err := container2.NewClientWithCluster(cluster)
...
func TestContainerResource(t *testing.T) {
	...
	t.Run("apply docker container", func(t *testing.T) {
		...
		id, err := client.RunContainer(&container2.CreateOptsForContainer{
		...
		deleteNetErr := client.DeleteNetworkResource(client.Cluster.Annotations[container2.NETWROKID])
		deleteImageErr := client.DeleteImageResource(client.Cluster.Annotations[container2.IMAGEID])
...
func CheckContainerApplyResult(cluster *v1.Cluster) bool {
	...
	if cluster.Annotations[container2.IMAGEID] == "" ||
		cluster.Annotations[container2.NETWROKID] == "" ||
