...
import (
	...
	"encoding/json"
	"fmt"
	types2 "github.com/alibaba/sealer/pkg/image/types"
...
type Backend interface {
	...
	setImageMetadata(metadata types2.ImageMetadata) error
	getImageMetadataItem(nameOrID string) (types2.ImageMetadata, error)
...
type ImageMetadataMap map[string]types2.ImageMetadata
...
func (fs *filesystem) getImageMetadataItem(nameOrID string) (types2.ImageMetadata, error) {
	imageMetadataMap, err := fs.getImageMetadataMap()
	imageMetadata := types2.ImageMetadata{}
	if err != nil {
	...
	return imageMetadata, &types2.ImageNameOrIDNotFoundError{Name: nameOrID}
}

func (fs *filesystem) setImageMetadata(metadata types2.ImageMetadata) error {
	imagesMap, err := fs.getImageMetadataMap()
	if err != nil {
		return err
	}
...
func (fs *filesystem) saveImage(image v1.Image, name string) error {
	...
	return fs.setImageMetadata(types2.ImageMetadata{Name: name, ID: image.Spec.ID})
}
