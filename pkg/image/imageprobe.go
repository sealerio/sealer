...
type Prober interface {
	Reset()
	Probe(parentID string, layer *cache2.Layer) (cacheID digest.Digest, err error)
}
...
func (c *imageProber) Probe(parentID string, layer *cache2.Layer) (cacheID digest.Digest, err error) {
	if c.cacheBusted {
		return "", nil
	}
...
func (c *nopProber) Probe(_ string, _ *cache2.Layer) (digest.Digest, error) {
