...
import (
	"context"
	"fmt"
	distributionutil2 "github.com/alibaba/sealer/pkg/image/distributionutil"
...
func (d DefaultImageService) Pull(imageName string) error {
	...
	puller, err := distributionutil2.NewPuller(named, distributionutil2.Config{
...
func (d DefaultImageService) Push(imageName string) error {
	...
	pusher, err := distributionutil2.NewPusher(named,
		distributionutil2.Config{
			LayerStore:     layerStore,
			ProgressOutput: progressChanOut,
		})
	if err != nil {
...
func (d DefaultImageService) Login(RegistryURL, RegistryUsername, RegistryPasswd string) error {
	err := distributionutil2.Login(context.Background(), &types.AuthConfig{ServerAddress: RegistryURL, Username: RegistryUsername, Password: RegistryPasswd})
	if err != nil {
...
func (d DefaultImageService) Delete(imageName string) error {
	...
	err = store2.DeleteImageLocal(image.Spec.ID)
	if err != nil {
		return err
	}
	...
	for _, layer := range image.Spec.Layers {
		layerID := store2.LayerID(layer.ID)
...
func isLayerDeletable(layer2ImageNames map[store2.LayerID][]string, layerID store2.LayerID) bool {
...
func layer2ImageMap(images []*v1.Image) map[store2.LayerID][]string {
	var layer2ImageNames = make(map[store2.LayerID][]string)
