...
func (puller *ImagePuller) Pull(ctx context.Context, named reference.Named) (*v1.Image, error) {
	...
	for i, l := range manifest.Layers {
		...
		eg.Go(func() error {
			...
			roLayer, LayerErr := store2.NewROLayer(layer.ID,
				descriptor.Size,
...
func (puller *ImagePuller) downloadLayer(ctx context.Context, named reference.Named, layer store2.Layer, descriptor distribution.Descriptor) error {
	var (
		layerStore  = puller.config.LayerStore
		progressOut = puller.config.ProgressOutput
		repo        = puller.repository
	)
	backend, err := store2.NewFSStoreBackend()
	if err != nil {
		return err
	}
